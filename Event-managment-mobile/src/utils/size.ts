...
export function Size(size: number) {
  ...
  if (SCREEN_WIDTH < SCREEN_HEIGHT) {
    ...
    if (SCREEN_WIDTH >= 950) return widthPixel(size) * 0.63;
...
export function keyboardOffset(size: number, insets = 0) {
  let res = Size(SCREEN_WIDTH > 410 ? size + 15 : SCREEN_WIDTH > 390 ? size + 25 : size);
  if (SCREEN_WIDTH < SCREEN_HEIGHT) res = insets + res;
  if (SCREEN_WIDTH >= 700) {
    res = res - Size(65);
  } else {
    if (SCREEN_WIDTH > SCREEN_HEIGHT) res = 0;
  }
  return res;
}
